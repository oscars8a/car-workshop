...
class Task(models.Model):
    ...
    #     for record in self:
    #         record['sale_order_id.currency_id'] = self.partner_id
    #
    # sale_order_id.currency_id= _default_currency_id()

    # @api.model
    # def _default_warehouse_id(self):
    #     company = self.env.user.company_id.id
    #     warehouse_ids = self.env['stock.wareho use'].search([('company_id', '=', company)], limit=1)
